...
std::vector< std::vector<double> > csaAssign (const int n, 
    ...
    std::vector< std::vector<double> > points(e,std::vector<double> (3,0));
    for (int i = 0; i < e; i++) {
        int a, b, cost;
        csa.edge(i,a,b,cost);
        points[i].resize(3);
...
//     out_points = csaAssign (n, double* g, m, m);

//     // Construct result.
//     plhs[0] = mxCreateDoubleMatrix(3, out_points.size(), mxREAL);
//     double* points = mxGetPr(plhs[0]);
//     for (i =0;i < 5; i++) {
//       std::copy(out_points[i].begin(), out_points[i].end(), points);
//       points += out_points[i].size();
//     }
