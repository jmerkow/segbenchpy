...
inline double interp( const double *I, int h, int w, double x, double y ) {
  x = x<0 ? 0 : (x>w-1.001 ? w-1.001 : x);
  y = y<0 ? 0 : (y>h-1.001 ? h-1.001 : y);
  int x0=int(x), y0=int(y), x1=x0+1, y1=y0+1;
  double dx0=x-x0, dy0=y-y0, dx1=1-dx0, dy1=1-dy0;
...
void edgeNms2d(const double *E0, int h, int w,
 ...
 for(int x=0; x<w; x++ ) for( int y=0; y<h; y++ ) {
    double e=E[x*h+y]=E0[x*h+y]; if(!e) continue; e*=m;
    double coso=cos(O[x*h+y]), sino=sin(O[x*h+y]);
    for( int d=-r; d<=r; d++ ) if( d ) {
      double e0 = interp(E0,h,w,x+d*coso,y+d*sino);
